...
- 지도 학습: 명시적인 답이 주어진 상태로 기계학습
  ...
  - 분류(Classification): 범주 라벨에 대한 모델 예측
    ...
    - 혼동행렬(Confusion Matrix): 예측과 실제 값 사이의 관계를 행렬 형태로 표현한 것.
      ...
      - 재현율(sensitivity, recall): $\frac{TP}{TP + FN}$
      - F1-score: 정밀도와 재현율의 조화평균, $F1 = 2*\frac{precision * sensitivity}{precision + sensitivity}$
  - 학습 모델의 성능 평가
    - 오버피팅(overfitting, 과적합): 훈련 데이터의 우연한 패턴/잡음까지 외워버려서 훈련에서는 잘 맞지만 테스트에서는 성능이 나빠지는 현상(너무 복잡한 모델).
...
- LLM의 학습 방식: Pre-training vs Post-training
  - Pre-training(사전 학습): 방대한 인터넷 텍스트를 통해 언어와 지식을 배우는 단계
    - 방대한 인터넷 텍스트 데이터를 이용한 Self-supervised learning을 통해 언어 패턴, 지식 등을 배움으로써 다음 단어 예측(next token prediction)이 가능케 한다.
    - 다음 단어를 예측하는 것에 강점을 보이긴 하지만 질문에 대한 대답을 하는 것은 아니다.
  ...
  - Instruction-tuning
    - 사전 학습 후 LLM은 유저의 의도와 일치하지 않기 때문에 파인 튜닝을 진행할 필요가 있다.
      - 파인 튜닝: 이미 사전학습된 기존 모델에 특정 작업이나 도메인에 맞게 추가로 학습시키는 과정.
    - 언어 모델이 사람이 내린 지시문(instruction)을 따르도록 학습하는 단계. 정답 레이블이 요구되며, 다양한 태스크를 풀 수 있도록 적응하는 것에 중점을 둔다.
    - 다양한 태스크에서 (지시문, 응답) 쌍을 모아서 언어 모델을 파인튜닝하고, 새로운(모델이 학습하지 않은) 태스크에서 평가를 진행한다. - 데이터와 모델의 크기가 핵심이다.
    - 한계점
      - 정답 데이터를 수집하는 비용이 비싸다.
      - 개방형/창의적 생성과 같은 태스크에는 정답이 존재하지 않는다.
      - 언어 모델링은 모든 토큰 레벨의 오류를 동일하게 취급하지만, 실제로는 그렇지 않다.(어떤 오류는 다른 오류보다 심하게 작용한다.)
      - 사람이 만든 답변(정답 레이블)이 최적이 아닐 수도 있다.
  ...
  - RLHF(Reinforcement Learning from Human Feedback)
    - 인간의 선호를 반영한 최적화(Optimizing for human preferences)
      - 언어모델을 학습하는 과정에서 어떠한 지시문(instruction) x와 언어모델의 응답 y가 있을 때, 해당 요약에 대한 인간이 준 보상(human reward)을 얻을 수 있게 하고 언어모델의 응답 중 기대 보상(expected reward)를 최대화 하는 것을 목표로 한다.
    - 흐름: instruction-tuning -> 보상 최대화
    - 인간의 판단은 일관성이 떨어지고 기준이 어긋날 수 있기 때문에, 직접 점수를 매기는 것이 아닌 응답을 비교하는 방식을 활용한다.
    - 한계점
      - 인간의 선호도는 일관성이 부족하다. - reward hacking
      - 챗봇들은 정답의 여부와 관계없이 생산적이고 도움이 되어 보이는 정답을 생성하게 된다. - 환각 문제를 야기함
  - DPO(Direct Preference Optimization): RLHF에서 RL(강화학습)을 제거하기
  - RLVR(Reinforcement Learning with Verifiable Reward): 수학 문제와 같이 답이 분명한 문제들은 정답 여부로 리워드를 주기

---
검색증강 언어모델(Retrieval-augmented LM)
- 추론 시 외부 데이터 저장소를 불러와 활용하는 언어 모델
- 구성 요소: datastore, query, index, language model
  - Datastore: 가공되지 않은 대규모 텍스트 코퍼스(corpus, 목적성을 갖고 체계적으로 모아 놓은 데이터 또는 언어의 집합)
    - 최소 수십 억~ 수 조 단위의 토큰으로 구성된다.
    - 라벨링되지 않고 구조화되지 않은 데이터의 집합이다.
  - Query: 검색 질의(retrieval input): 언어모델의 질의와 같아야 하는 것은 아니다.
  - Index: 문서나 단락과 같은 검색 가능한 항목들을 체계적으로 정리하여 더 쉽게 찾을 수 있도록 하는 것. 각 정보 검색(information retrieval) 메서드는 인덱싱 과정에서 구축된 인덱스를 활용해, 쿼리와 관련 있는 정보를 식별한다.
  - Retrieval: datastore에 있는 수 많은 정보 중, 주어진 query와 가장 관련성이 높은 정보를 찾아내는 과정.
  - RAG(Retrieval-Augmented Generation): 사용자의 질문에 답하기 위해, datastore에서 관련 정보를 retrieval하여, 이를 언어모델이 생성 단계에 함께 활용하는 방법.
- Information Retrieval(IR, 정보검색)
  - 사용자의 질의에 맞는 정보를 대규모 데이터에서 찾아 제공하는 과정. 사용자의 검색 질의에 가장 관련성이 높은 정보를 제공하는 것을 목표로 한다.
  - 웹/아이템 서치, 추천 시스템, 검색 증강 생성(RAG) 등에 활용된다.
  - Retriever: 사용자의 질의에 맞는 후보 문서를 저장소에서 찾아오는 모듈.
    - Sparse Retriever(어휘적 유사도 기반): 전통적인 정보검색 기법으로, 쿼리와 문서 간의 정확한 용어 일치에 기반한다.
      - TF-IDF(Term Frequency-Inverse Document Frequency): 문서 내 특정 단어의 중요도를 나타내는 가중치 방식. 
        - TF: 단어가 얼마나 문서에 자주 등장하는가 -> 문서 안에서 자주 등장하는 단어는 중요하다.
        - IDF: 단어가 전체 코퍼스에서 얼마나 드물게 등장하는가 -> 너무 많은 문서에서 등장하는 단어는 덜 중요하다.
      - 장점
        - 단순성: 구현과 이해가 비교적 쉽다.
        - 효율성: Inverted index 구조 덕분에 빠른 검색과 효율적인 질의 처리가 가능하다.
        - 투명성: 검색 결과가 보통 해석 가능하며, 용어 매칭에 기반하기 때문에 설명이 명확하다.
      - 단점
        - 제한된 의미 이해(limited semantic understanding): 쿼리와 실제 문서에 같은 의미에 단어가 포함되었더라도 어휘가 다르면 매칭되지 않을 가능성이 있다.
    - Dense Retriever(의미적 유사도 기반): 쿼리와 문서를 표현하기 위해 dense vector(임베딩)을 활용해 의미적 유사도에 기반한다.
      - 임베딩 모델: 단어와 문장의 의미를 표현할 수 있다.
      - Bi-encoder retriever: 대조 학습(contrastive learning)을 통해 학습되며, 이는 쿼리가 긍정적인 문서와 가깝게 유지되도록 하고 부정적인 문서에서는 멀어지도록 한다.
      - Cross-encoder: 두개의 텍스트를 하나의 시퀀스로 결합한다.
        - self-attention을 통해 모든 쿼리와 문서 토큰이 완전히 상호작용할 수 있어, bi-encoder보다 더 높은 정확도를 얻을 수 있다.
        - 하지만 모든 쿼리-문서 쌍을 개별적으로 모델에 입력해야하므로, 계산 비용이 크고 처리속도가 느리다.
      - bi-encoder vs cross-encoder
        - bi-encoder는 두 문장을 따로 인코딩한다: 매우 빠르고 대규모 데이터베이스 검색에 적합하지만 정확도는 낮다.
        - cross-encoder는 두 문장을 함께 처리한다: 매우 느리지만, 세밀한 상호작용을 포착하기 때문에 정확도가 높다.
      - 장점
        - 의미적 이해: 동의어나 다양한 표현을 더 효과적으로 처리할수 있다, 즉 글의 문맥과 의미를 포착한다.
        - 풍부한 쿼리 표현: 복잡한 쿼리와 긴 검색 쿼리의 의미를 더 잘 포착할 수 있다.
      - 단점
        - 높은 연산 비용
        - 제한된 투명성: dense retriever는 블랙박스처럼 작동할 수 있어, 특정 문서가 왜 검색되었는지 해석하기 어렵다.
        - 데이터 및 모델 의존성: dense retriever의 성능은 학습 데이터의 품질이나 모델 변경에 크게 영향을 받는다.
  - Retrieval-augmented LM
    - RAG(Retrieval-Augmented Generation): 추론 시 외부 데이터 저장소를 불러와 활용하는 언어모델, 정보 검색부터 답변 생성까지의 프레임워크
    - 의의
      - 거대언어모델은 모든 지식을 다 자신의 파라미터에 저장하지 못한다.
        - 사전학습 데이터에 자주 나타나는 쉬운 정보를 기억하는 경향성이 있다.
        - RAG는 자주 등장하지 않는 정보에 대해서 큰 효과를 가져다 준다. 단, 언어 모델이 잘 알고 있는 정보인 경우 부정적인 영향을 미칠 수도 있다.
      - 거대언어모델이 보유한 지식은 금세 시대에 뒤쳐지며 갱신이 어렵다.
        - 현재의 지식 편집 메서드들은 확장성이 부족한 반면, datastore는 쉽게 업데이트가 가능하며 확장성도 지니고 있다.
      - 거대언어모델의 답변은 해석과 검증이 어렵다.
      - 기업 내부 정보와 같은 정보는 언어모델 학습에 활용되지 않는다.
        - 사내 챗봇/기업 내부 시스템에 언어모델을 사용하는 경우 내부 데이터를 학습 시 정보 유출의 위험성이 있다.
    - 파이프라인: 질의 추출 -> 문서 검색 -> 언어모델 추론
      - 언어모델에 질문과 더불어 검색엔진 결과를 함께 이용한다.
    - 한계
      - RAG의 결과는 검색 모델 성능에 의존한다: 검색 노이즈에 취약하고 환각의 증가 가능성이 있다.
      - 모델의 사전지식과 컨텍스트 간의 충돌 발생
      - 복잡한 추론이 필요한 경우와 문서가 명확한 사실에 대한 오류를 포함한 경우에 취약하다.
  
---
LLMs with Tool Usage
- Agent: 센서를 통해 환경을 인지하고, actuator를 통해 환경에 대해 액션을 통해 영향을 미치는 것으로 간주될 수 있는 모든 것.
- LLM Agent: 거대언어모델을 핵심구조(backbone)로 삼아 환경을 이해하고 행동을 수행하는 에이전트
  - LLM-first view: 기존 LLM을 활용한 시스템을 에이전트로 만든다.
  - Agent-first view: LLM을 AI 에이전트에 통합하여 언어를 활용한 추론과 의사솥ㅇ을 가능하게 한다.
  - Agent의 포함 요소: 도구 사용(tool use), 추론과 계획(reasoning and planning), 환경 표현(environment representation), 환경 이해(environment understanding), 상호작용/의사소통(interaction/communication)
- Tool usage in LLM: 언어 모델 외부에서 실행되는 프로그램에 연결되는 함수 인터페이스. LLM은 함수 호출과 입력 인자를 생성함으로써 이 도구를 활용할 수 있다.
  - 도구 사용: 두 모드 간의 전환으로 이루어진다.
    - 텍스트 생성 모드
    - 도구 실행 모드
  - tool learning 방식
    - 모방 학습(imitation learning): 인간의 도구 사용 행동 데이터를 기록함으로써, 언어모델이 인간의 행동을 모방하도록 학습시키는 가장 간단하고 직관적인 방식.
      - 장문 질의응답에서 뛰어난 성능을 보이며, 인간보다 좋은 성능을 보이기도 한다.
    - 멀티 모달 툴 러닝(Mutil-modal tool learning): 멀티모달 대규모 언어모델(MLLM)을 기반으로 도구를 정의하고 활용하도록 하는 최근에 이루어지고 있는 연구. + 강화 학습
- MCP(Model Context Protocol)
  - 외부 툴을 활용하는 연구가 급증하면서 회사/모델마다 각기 다른 출 호출 방식 및 스키마를 개발 -> 호환성 부족과 재사용이 어렵다는 문제를 야기함
  - MCP는 언어 모델이 외부 툴과 상호작용하기 위한 표준화된 방식으로 정의한 프로토콜. 툴 호출, 응답 전달, 컨텍스트 공유를 하나의 공통 규격으로 처리한다.
  - 아키텍쳐
    - MCP host: 하나 또는 여러개의 MCP 클라이언트를 조정하고 관리하는 ai 애플리케이션
    - MCP client: MCP 서버와의 연결을 유지하며 MCP 호스트가 사용할 수 있도록 MCP 서버로부터 컨텍스트를 가져오는 구성요소
    - MCP server: MCP 클라이언트에게 컨텍스트를 제공하는 프로그램
  - 계층
    - 데이터 계층(data layer): 클라이언트-서버 통신을 위한 json-RPC 기반 프로토콜을 정의한다. (inner layer)
    - 전송 계층(transport layer): 클라이언트 서버 간 데이터 교환을 가능하게 하는 통신 메커니즘과 채널을 정의한다. (outer layer)
  - 장점
    - 표준화: 모든 모델/툴이 동일한 호출 규격을 사용
    - 확장성: 새로운 툴을 추가하기 쉬움
    - 호환성: 모델/플랫폼에 상관없이 같은 툴을 호출 가능
...
Langchain
- 에이전트의 환경 표현과 이해
  - 환경의 표현: 텍스트, 이미지, 텍스트 기반 웹
  - 환경의 이해: 모델은 자신이 상호작용하는 환경에 대해 일부 지식은 LLM 파라미터 안에 포함되어 있으나, 모든 것을 알고 있을 수는 없다. 이러한 부족한 부분은 실시간으로 환경과의 상호작용을 통해 발견해야 한다.
    - 환경 특화 프롬프트(environment-specific prompt): 환경에 맞게 수동으로 프롬프트를 제작하여 에이전트가 지시를 따르도록 유도한다. - 특정 환경에서는 잘 작동하지만, 새로운 환경에서는 성능이 떨어진다.
    - 비지도 프롬프트 유도(unsupervised induction of prompt): 성공적으로 수행된 워크플로우를 기억하고, 이를 기반으로 새로운 프롬프트를 생성하여 에이전트에 제공한다. 프롬프트를 사람이 수동으로 설계하지 않고, 에이전트가 경험을 통해 자동 생성 및 일반화한다.
    - 환경 탐색(environment exploration): 모델이 환경을 탐색할 때 보상을 부여하여 학습을 유도한다.(호기심 기반 보상)
    - 탐색 기반 궤적 기억(exploration-based trajectory memorization): 명령어를 샘플링하여 실행한뒤, 더 정확한 새로운 명령어로 궤적(trajectory)을 재라벨링하면 LM-agent가 환경을 탐색하고 LM-labeler가 이를 바탕으로 지시를 다시 정제한다. - 기존 데이터에 의존하지 않고, 탐색과 자기 교정을 통해 데이터를 생성하여 에이전트의 일반화 성능과 환경 적응력을 향상시킨다.
- 추론과 계획
  - 계획(planning)
    - 국소적 계획(local planning)
      - 한단계씩 계획을 세움
      - 매 스텝마다 사용할 하나의 툴을 결정
      - 단순하고 직관저깅나, 장기 의존성 문제 발생
    - 전역적 계획(global planning)
      - 실행 가능한 전체 계획 경로를 한번에 생성
      - 여러 개의 툴을 조합하여 시퀀스 형태로 결정
      - 효율적이긴 하나, 복잡한 환경에서는 실패 가능성이 있다.
  - 오류 식별과 회복
    - reflection
      - 수행한 궤적을 평가한 후 잘못된 부분을 reflection 단계에서 분석하여 재시도하는 방식
      - 단순히 오류를 탐지하는 것에 그치지 않고, 스스로 반성 후 개선된 행동을 수행한다.
      - 의사 결정, 프로그래밍, 추론 등 다양한 태스크에서 성능 회복 및 향상을 보여준다.
    - 계획 재검토 - CoAct
      - 에이전트가 실행 도중 계획을 재검토하고 수정 가능
      - 두개의 에이전트가 서로 협력하여 오류가 발생시 재계획과 피드백을 수행한다.
      - 초기 계획이 불완전하더라도 실행 과정에서 보완이 가능하다.
      - 장기 작업에서 안전성과 성공률 향상.
- Langchain: LLM 기반 애플리케이션을 빠르게 개발할 수 있는 오픈소스 프레임워크

---
파인튜닝 및 AI모델 활용: 리소스-효율적 AI모델
- 컴퓨터의 수 체계
  - 이진수
    - MSB(Most Significant Bit or Byte): 이진수에서 가장 왼쪽에 있는(자릿값이 가장 큰) 비트(바이트)
    - LSB(Least Significant Bit or Byte): 이진수에서 가장 오른쪽에 있는(자릿값이 가장 작은) 비트(바이트)
    - Nibble: 4비트(half-byte)로 이루어진 데이터 단위. (잘 안씀)
  - 십육진수: 이진수와 간편한 호환이 가능
  - 부호가 있는 수 체계(signed number system)
    - 부호-크기 표현 방식(signed-magnitude representation): 최상위 비트를 부호 비트로 사용하여, 0이면 양수, 1이면 음수를 나타내고 나머지 비트는 절댓값을 표현하는 방식.
      - 음수와 양수간의 덧셈에서 문제가 발생한다.
      - 0을 표현하는 방식에 두가지가 존재해버린다.
    - 2의 보수 표현 방식(two's complement representation): 음수를 표현할 떼, 해당 수의 절댓값을 이진수로 쓰고 모든 비트를 반전한 뒤 1을 더하는 방식
      - 기존 덧셈이 정상적으로 작동한다.
  - 비트 폭 확장(Increasing Bit Width): N-bit 수를 M-bit 수로 확장(M > N)
    - 부호 확장(sign extension): 최상위 비트를 복사해 상위 비트를 채운다.(값을 일정하게 유지)
    - 제로 확장(zero extension): 상위 비트를 모두 0으로 채운다.(패턴을 일정하게 유지)
  - 정수의 자료형
    - int
      - 가장 효율적으로 처리될 수 있는 정수 타입을 목표로 정의됨
      - 초기에는 컴퓨터 구조에 따라서 다른 크기였으나 64-bit 구조가 등장하면서 혼란을 막기위해 int 자료형은 32-bit 자료형으로 고정되었다.
      - C언어에서의 엄밀한 정의는 조금 다를 수 있으나, 64-bit 구조 이후로는 4B로 길이가 고정된다.
    - long
      - 과거의 int 자료형처럼 컴퓨터/OS 구조에 따라 4B or 8B로 사용
      - window 환경에서는 int나 long long(8B) 권장
  - 고정소수점 수 체계(fixed-point number system)
    - 정수부와 소수부를 N-bit 패턴에서 함께 표현하는 것이 가능하다
    - 표현에 많은 비트를 요구
  - 부동소수점 수 체계(floating-point number system)
    - 지수를 활용하여 큰/작은 숫자를 더 적은 문자로 표현하기: 유효숫자의 개념을 도입 (n.nnn... * 10^n)
    - 소수점이 고정되지 않는다. (부동(浮動) - 떠다니다)
  - 이진 부동소수점 수 체계
    - 지수적 표현을 이진 수 체계에 접목: $F = (-1)^S \cdot M \cdot \beta^E$
      - S: 부호(1 bit)
      - E: 지수(e bit)
      - M: 부호 없는 가수(s bit)
        - $[0, 1)$: 순수한 소수 표현의 가수
        - $[1, 2)$: 정규화된 표현의 가수
      - $\beta$: 밑. 일반적으로 2의 거듭제곱 형태를 선택
    - 일반적으로 0으로 시작하는 패턴을 피하고 유효숫자를 최대한 확보한다.(정규화)
    - $\beta=2^k$ 에서 가수는 k 비트 단위로 이동 가능 -> 첫 k비트가 0이 아니라면 정규화 된 것으로 본다.
    - 따라서, 정규화된 표현은 0을 포함할 수 없다.
